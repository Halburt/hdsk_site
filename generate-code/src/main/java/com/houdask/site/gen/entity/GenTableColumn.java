...
public class GenTableColumn extends DataEntity<GenTableColumn> {
	...
	public List<String> getAnnotationList2(){
		List<String> list = Lists.newArrayList();
		if("uuid".equalsIgnoreCase(name)){
			list.add("Id");
			list.add("GeneratedValue(generator = \"paymentableGenerator\")");
			list.add("GenericGenerator(name = \"paymentableGenerator\",strategy = \"assigned\")");
			return list;
		}
		// 导入Jackson注解
		if ("This".equals(getJavaType())){
		...
		if ("java.util.Date".equals(getJavaType())){
		   /* @Temporal(TemporalType.DATE)
			...
			list.add("Temporal(TemporalType.DATE)");
		}
		String c = "Column(name=\""+name.toUpperCase()+"\"";
		// 导入JSR303验证依赖包
		if (!"1".equals(getIsNull()) && !"String".equals(getJavaType())){
			c += ",nullable = true";
		}
